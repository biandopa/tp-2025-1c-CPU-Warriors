...
func (m *Memoria) BuscarFrame(pid int, entradasPorNivel string) (DirInfoResponse, error) {
	url := fmt.Sprintf("http://%s:%d/cpu/pagina-a-frame?pid=%d&entradas-nivel=%s",
		m.IP, m.Puerto, pid, entradasPorNivel)
